// Dev-only offline aids: let the full screen flow run with NO API keys.
// Used by POST /api/mock-process and the no-key branch of /api/buyer.
// These never run when NEBIUS_API_KEY is set (real path takes over).

// Mirrors public/demo-fixture.js so the offline path looks like the live one.
export const MOCK_CALL = {
  dossier: {
    ...
    signature_takes: [
      "AI design tools should be opinionated, not blank-canvas",
      "Design systems are the only place AI codegen is safe to ship today",
    ],
...
// Deterministic shortlist from the seed pool, so the buyer flip works offline.
export function mockShortlist(pool, limit = 3) {
  return (pool.profiles ?? []).slice(0, limit).map((p) => ({
    profile_id: p.id,
    name: p.name,
    why: `${(p.subspecialties ?? [])[0] ?? "their work"} — "${(p.signature_takes ?? [])[0] ?? ""}"`,
  }));
}
